...
import ChongzhixinxiModel from '../models/ChongzhixinxiModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			let chongzhiriqistart = req.query.chongzhiriqistart;
			let chongzhiriqiend = req.query.chongzhiriqiend;
			if (chongzhiriqistart && chongzhiriqiend) {
				let chongzhiriqibetween = [];
				chongzhiriqibetween.push(chongzhiriqistart);
				chongzhiriqibetween.push(chongzhiriqiend);
				where.chongzhiriqi = {
					[Op.between]: chongzhiriqibetween
				}
			}
			let chongzhibianhao = req.query.chongzhibianhao
			if (chongzhibianhao) {

				if (chongzhibianhao.indexOf('%') != -1) {
					where.chongzhibianhao = {
						[Op.like]: chongzhibianhao
					}
				} else {
					where.chongzhibianhao = {
						[Op.eq]: chongzhibianhao
					}
				}
			}
			...
			let chongzhiriqi = req.query.chongzhiriqi
			if (chongzhiriqi) {

				if (chongzhiriqi.indexOf('%') != -1) {
					where.chongzhiriqi = {
						[Op.like]: chongzhiriqi
					}
				} else {
					where.chongzhiriqi = {
						[Op.eq]: chongzhiriqi
			...
			let chongzhibeizhu = req.query.chongzhibeizhu
			if (chongzhibeizhu) {

				if (chongzhibeizhu.indexOf('%') != -1) {
					where.chongzhibeizhu = {
						[Op.like]: chongzhibeizhu
					}
				} else {
					where.chongzhibeizhu = {
						[Op.eq]: chongzhibeizhu
					}
				}
			}
			...
			let result = await ChongzhixinxiModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await ChongzhixinxiModel.findAll()
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await ChongzhixinxiModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await ChongzhixinxiModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await ChongzhixinxiModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await ChongzhixinxiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM chongzhixinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM chongzhixinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM chongzhixinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM chongzhixinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM chongzhixinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM chongzhixinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "chongzhixinxi"
			let where = " WHERE 1 = 1 "
			let tableName = req.session.userinfo === undefined ? jwt.decode(req.headers.token).tableName : req.session.userinfo.tableName
			if(tableName == 'xueyuan') {
				where += " AND zhanghao = '" + jwt.decode(req.headers.token).username + "' ";
			}
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM chongzhixinxi " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("chongzhixinxi" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM chongzhixinxi " + where + " GROUP BY " + xColumnName + " DESC"
