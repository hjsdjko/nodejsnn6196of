<template>
<div class="content" :style='{"minHeight":"calc(100vh - 70px)","padding":"0px 30px 20px","overflow":"hidden","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20231107/94f950da136d4785ad6630571b544668.png) no-repeat center top / cover","display":"flex","fontSize":"14px","justifyContent":"space-between","height":"100%"}'>
	<!-- notice -->
	<!-- title -->
	...
	<div :style='{"border":"0px solid rgba(126, 96, 16, .1)","padding":"0px 0","margin":"0 0 10px","alignItems":"center","color":"#1e3c4f","display":"flex","float":"left","justifyContent":"center","flexWrap":"wrap","background":"none","width":"49%","fontSize":"14px","order":"0"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"12px 0","margin":"0 0 20px 0","borderColor":"rgba(126, 96, 16, .1)","borderRadius":"8px","alignItems":"center","textAlign":"left","background":"url(http://codegen.caihongy.cn/20231107/38670e9252144f0e92e9875cffa339b5.png) no-repeat 96% bottom / auto 100%,#ff6746","borderWidth":"0 0 0px","display":"flex","width":"100%","borderStyle":"solid"}' v-if="isAuth('changdixinxi','首页总数')">
			...
			<div :style='{"padding":"0 0 6px","borderColor":"#fff","flexDirection":"row","borderWidth":"0 0 2px","display":"flex","width":"50%","borderStyle":"solid","justifyContent":"flex-start"}'>
				<div :style='{"margin":"5px 20px 0 0","lineHeight":"24px","fontSize":"34px","color":"#fff","fontWeight":"bold","height":"24px"}'>{{changdixinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"#fff","height":"24px"}'>场地信息总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"30px 20px","margin":"0 4% 20px 0","borderColor":"rgba(126, 96, 16, .1)","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"space-between","borderRadius":"8px","background":"#fff","borderWidth":"0 0 0px","width":"48%","borderStyle":"solid"}' v-if="isAuth('dingyuexinxi','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"row","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 20px 0 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{dingyuexinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>订阅信息总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"30px 20px","margin":"0 0 20px 0","borderColor":"rgba(126, 96, 16, .1)","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"space-between","borderRadius":"8px","background":"#fff","borderWidth":"0 0 0px","width":"48%","borderStyle":"solid"}' v-if="isAuth('goumaikeshi','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"row","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 10px 0 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{goumaikeshiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>购买课时总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type4" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto","order":"10"}'>
		<div id="changdixinxiChart1" class="echarts1" v-if="isAuth('changdixinxi','首页统计')"></div>
		<div id="changdixinxiChart2" class="echarts2" v-if="isAuth('changdixinxi','首页统计')"></div>
		<div id="dingyuexinxiChart1" class="echarts3" v-if="isAuth('dingyuexinxi','首页统计')"></div>
		<div id="goumaikeshiChart1" class="echarts4" v-if="isAuth('goumaikeshi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.changdixinxiChat2();
		this.getdingyuexinxiCount();
		this.dingyuexinxiChat1();
		this.getgoumaikeshiCount();
		this.goumaikeshiChat1();
	},
	methods:{
		// 词云
        ...
        var changdixinxiChart1 = echarts.init(document.getElementById("changdixinxiChart1"),'fresh-cut');
        this.$http({
    ...
    changdixinxiChat2() {
      this.$nextTick(()=>{
        ...
        var changdixinxiChart2 = echarts.init(document.getElementById("changdixinxiChart2"),'fresh-cut');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                changdixinxiChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    changdixinxiChart2.resize();
                };
            }
        });
      })
        ...
        var dingyuexinxiChart1 = echarts.init(document.getElementById("dingyuexinxiChart1"),'fresh-cut');
        this.$http({
            url: `dingyuexinxi/value/changdimingcheng/dingyueshizhang`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].changdimingcheng);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].changdimingcheng
                    })
                }
                var option = {};
		...
		getgoumaikeshiCount() {
			this.$http({
				url: `goumaikeshi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.goumaikeshiCount = data.data
				}
			})
		},
		goumaikeshiChat1() {
        ...
        var goumaikeshiChart1 = echarts.init(document.getElementById("goumaikeshiChart1"),'macarons');
        this.$http({
            url: `goumaikeshi/value/xingming/keshi`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                goumaikeshiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    goumaikeshiChart1.resize();
                };
            }
        });
      })
