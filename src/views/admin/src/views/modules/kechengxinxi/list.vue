<template>
	<div class="main-content" :style='{"padding":"0px 30px 30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"flex-end"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 10px 20px","borderRadius":"0 0 8px 8px","textAlign":"right","background":"none","display":"block","width":"auto","fontSize":"inherit","order":"1"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="上课地点" prop="shangkedidian">
						...
						<el-select clearable v-model="searchForm.shangkedidian" placeholder="请选择上课地点" >
							<el-option v-for="(item,index) in shangkedidianOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','立即购买')" @click="goumaikeshiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<goumaikeshi-cross-add-or-update v-if="goumaikeshiCrossAddOrUpdateFlag" :parent="this" ref="goumaikeshiCrossaddOrUpdate"></goumaikeshi-cross-add-or-update>
...
import goumaikeshiCrossAddOrUpdate from "../goumaikeshi/add-or-update";
	export default {
		data() {
			return {
				...
				goumaikeshiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			goumaikeshiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.goumaikeshiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.goumaikeshiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.goumaikeshiCrossaddOrUpdate.init(row.id,type);
				});
			},
