...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"1"}'>
	  ...
	  <el-button v-if="btnAuth('kechengxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 25px","margin":"0 10px 20px 0","outline":"none","color":"#fff","borderRadius":"0","background":"#57A7A5","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/kechengxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"0 0 10px","background":"none","order":"4"}'>
		...
		<div :style='{"padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"margin":"20px 0 0","overflow":"hidden","background":"#fff","width":"32%","minWidth":"320px","position":"relative","height":"280px"}' class="item">
			<img @click.stop="imgPreView(item.kechengfengmian)" v-if="item.kechengfengmian && item.kechengfengmian.substr(0,4)=='http'" :src="item.kechengfengmian.split(',')[0]" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.kechengfengmian?item.kechengfengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.kechengfengmian?item.kechengfengmian.split(',')[0]:'')" class="image" />
			<div class="content">
		      <div :style='{"padding":"0","overflow":"hidden","color":"#fff","flex":"1","display":"-webkit-box","width":"50%","lineHeight":"24px","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"5","height":"120px"}'>{{item.kechengjianjie}}</div>
		      <div class="info" :style='{"width":"50%","padding":"0 0 0 10px","flexWrap":"wrap","flex":"1","display":"flex","height":"100%"}'>
				<div :style='{"overflow":"hidden","color":"#fff","display":"-webkit-box","width":"100%","lineHeight":"1.5","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"3","order":"1"}' class="name ">{{item.kechengmingcheng}}</div>
				<div :style='{"overflow":"hidden","color":"#fff","display":"-webkit-box","width":"100%","lineHeight":"1.5","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"3","order":"1"}' class="name ">课程费用:{{item.yue}}</div>
				<div :style='{"overflow":"hidden","color":"#fff","display":"-webkit-box","width":"100%","lineHeight":"1.5","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"3","order":"1"}' class="name ">课时:{{item.keshi}}</div>
		        ...
		        <div :style='{"padding":"0","display":"none"}'>
		          ...
		          <span :style='{"color":"#fff","lineHeight":"1.5","fontSize":"12px"}'>{{item.jiaolianzhanghao}}</span>
		        </div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/changdixinxi/changdimingcheng').then(res => {
			if (res.data.code == 0) {
				this.shangkedidianOptions = res.data.data;
				this.selectOptionsList.push({name:'上课地点',list:this.shangkedidianOptions,tableName: 'shangkedidian',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.shangkedidian != '') searchWhere.shangkedidian = this.formSearch.shangkedidian;
